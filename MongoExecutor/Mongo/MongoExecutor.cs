...
namespace MongoExecutor.Mongo
{
	public class MongoExecutor : IDisposable
	{
		...
		public string ExecuteCommand(string url, string filter, string document)
		{
			try
			{
				this.ReadyToConnect(url);
				...
				this._messageBag.TryTake(out var message, 5000); // 대기.
		...
		private void ReadyToConnect(string url)
		{
			if (this._mongo != null &&
				string.CompareOrdinal(this.Url, url) == 0 &&
				string.CompareOrdinal(this.Script, this._lastScript) == 0)
			{
				return;
			}
		...
		private void SaveQueryFile()
		{
			try
			{
				...
				this._lastScript = functionString;
			}
			catch (Exception ex)
			{
		...
		private void OnShellDataReceived(object sender, DataReceivedEventArgs e)
		{
			try
			{
				...
				if (e.Data.Contains(ScriptWrapper.EtxCompleted))
				{
				...
				if (e.Data.Contains(ScriptWrapper.Error) || e.Data.Contains(ScriptWrapper.Etx))
				{
					// etx. 완전히 받지 않음.
					this._hasLast = true;
					this._messageBuilder.AppendLine(e.Data);
